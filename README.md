...
每篇文章按抽取时的预估阅读时长换算成字数，保证进度条始终会动。

## 阅读原网页

点「打开」会直接在新标签页打开原网页。插件不读取、提取或改写网页正文，
也不需要任何网站访问权限。
...
`vite.config.ts` 构建阅读页、设置页与 background service worker。

### 测试

| 命令 | 内容 |
|---|---|
| `npm run smoke` | 批次状态机（34 项），含旧版本配置与阅读字数迁移 |
| `npm run e2e` | 真实 Chrome + 真实书签，21 项验证清单、原网页打开、权限、锁定、归档与放弃 |
...
```
src/
  background/service-worker.ts   入口调度、打开原网页、书签事件校正
  reader/                        阅读清单页（React）
  options/                       设置页（React）
  lib/
    batch.ts                     批次状态机（核心）
    ...
    messaging.ts
archive/
  focus-mode-v0.2.0/             已移除的专注阅读实现与集成快照
```
...
阅读时长按域名先验做粗略估算，用于卡片标签、时长均衡选文和街机分数换算。
